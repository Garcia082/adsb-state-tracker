...
# ---------- OAuth2 -------------------------------------------------
TOKEN_URL = ("https://auth.opensky-network.org/"
             "auth/realms/opensky-network/protocol/openid-connect/token")
...
def token():
    ...
    EXPIRES_AT = datetime.utcnow() + timedelta(seconds=j["expires_in"]-60)
    logging.info("Nuevo token recibido. Expira %s", EXPIRES_AT.isoformat())
    return TOKEN
# -------------------------------------------------------------------
...
INTERVAL = int(os.getenv("OPENSKY_INTERVAL", 60))   # segundos
...
def conn():
    return pymysql.connect(
        ...
        user=os.getenv("MYSQL_USER","root"),
        password=os.getenv("MYSQL_PASSWORD","root"),
        database="adsb", autocommit=True)

def es_estado(icao):            # ejemplo rápido
    return icao[:2].upper() in {"AE","AD","AF"}

def ciclo():
    while True:
        try:
            r = requests.get(API, headers={"Authorization":f"Bearer {token()}"}, timeout=15)
            if r.status_code == 429:              # rate-limit
                logging.warning("429 Too Many Requests – espero 2 min")
                time.sleep(120); continue
            r.raise_for_status()
            estados = r.json()["states"]
            with conn().cursor() as c:
                for s in estados:
                    if s[5] is None:   # sin lon
                        continue
                    c.execute("""REPLACE INTO aeronaves
                        (icao24,callsign,lat,lon,alt,vel,is_state,last_seen)
                        VALUES(%s,%s,%s,%s,%s,%s,%s,%s)""",
                        (s[0], (s[1] or "").strip() or None,
                         float(s[6]), float(s[5]),
                         int(s[13] or 0), int(s[9] or 0),
                         es_estado(s[0]), datetime.utcnow()))
            logging.info("Insertados %d registros", len(estados))
        except Exception as e:
            logging.exception("Error integrador: %s", e)
        time.sleep(INTERVAL)
